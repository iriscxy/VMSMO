...
class Example(object):
  ...
  def __init__(self, article, abstract_sentences, frames, max_sim, vocab, hps):
    ...
    # Process the sideinfo
    self.side_len = len(frames)
    self.side_input = frames
    self.target_pic = max_sim
  ...
  def pad_sideinfo_input(self, max_len, pad_id):
    """Pad the encoder input sequence with pad_id up to max_len."""
    while len(self.side_input) < max_len:
      self.side_input.append([pad_id] * 2048)
...
class Batch(object):
  ...
  def init_encoder_seq(self, example_list, hps):
    ...
    max_side_seq_len = hps.max_side_steps
    ...
    for i, ex in enumerate(example_list):
      ...
      for j in range(ex.side_len):
        ...
        if j % 5 == 0:
          self.segment_padding_mask[i][int(j/5)] = 1
...
class Batcher(object):
  ...
  def __init__(self, data_path, vocab, hps, single_pass):
    ...
    if single_pass:
      ...
      self._bucketing_cache_size = 10 # how many batches-worth of examples to load into cache before bucketing
  ...
  def fill_example_queue(self):
    ...
    while True:
      try:
        (article, abstract, frames, max_sim) = next(input_gen) # read the next example from file. article and abstract are both strings.
      ...
      example = Example(article, [abstract], frames, max_sim, self._vocab, self._hps) # Process into an Example.
  ...
  def fill_batch_queue(self):
    ...
    """
    while True:
      if self._hps.mode != 'decode' and self._hps.mode != 'auto_decode':
