...
class Weibo(object):
    def __init__(self, config):
        ...
        self.video_only = config['video_only'] #是否只保存含有video的微博
    ...
    def get_long_video_url(self, weibo_link):
        """获取长原创微博的video url"""
        try:
            for i in range(5):
                selector = self.handle_html(weibo_link)
    ...
    def get_original_weibo(self, info, weibo_id):
        ...
        try:
            ...
            if u'全文' in a_text or '全文' in weibo_content:
    ...
    def get_video_url(self, info, is_original, weibo_id):
        """获取微博视频url"""
        try:
            if is_original:
                div_first = info.xpath('div')[0]
                ...
                for a in a_list:
                    if a.text == u'全文':
                        weibo_link = 'https://weibo.cn/comment/' + weibo_id
                        video_link = self.get_long_video_url(weibo_link)
                        break
                    if 'm.weibo.cn/s/video/show?object_id=' in a.xpath(
                            '@href')[0]:
                        video_link = a.xpath('@href')[0]
                ...
                if video_link != u'无':
                    ...
                    video_url = wb_info['data']['object']['stream'].get(
                        'hd_url')
                    image_url = wb_info['data']['object']['image'].get(
                        'url')
                    if not video_url:
    ...
    def get_one_weibo(self, info):
        ...
        try:
            ...
            if (not self.filter) or is_original:
                ...
                weibo['video_url'], weibo['image_url'] = self.get_video_url(info,
                                                        is_original, weibo['id'])  # 微博视频url, 封面图片url
        ...
        try:
            ...
            if is_exist:
                for i in range(0, len(info) - 2):
                    ...
                    if weibo and weibo['video_url'] != u'无':
    ...
    def get_filepath(self, type):
        ...
        try:
            ...
            if type == 'img' or type == 'video_img' or type == 'video':
    ...
    def write_data(self, wrote_num):
        ...
        if self.got_num > wrote_num:
            ...
            if self.video_download == 1:
                ...
                self.download_files('video_img', wrote_num)
