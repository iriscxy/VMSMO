...
def dot_product_attention(queries,
    ...
    Args:
      ...
      mask: A ``tf.Tensor`` applied to the dot product. enc时候是[batch, 1, 1, len],dec train时候是[batch, 1, len, len]
...
def multi_head_attention(num_heads,
    ...
    if memory is None:  # 对queries做self attention
        ...
        if cache is not None:
            keys = tf.concat([cache["self_keys"], keys], axis=2)  # 在len维度进行concat 目的是最终记下来每一个decode step的key
            values = tf.concat([cache["self_values"], values], axis=2)  # 在len维度进行concat 目的是最终记下来每一个decode step的value
        ...
        if cache is not None:
            ...
            keys, values = tf.cond(
                ...
                # cache init的时候第二维是0，之后都不是0了，也就是这里只有第一decode步的时候是true，后面都是false
